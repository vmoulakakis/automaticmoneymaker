...
export default async function MonitorPage() {
  ...
  const [{ data: scores }, { data: runs }, { count: verifiedCount }, { count: productCount }] = await Promise.all([
    supabase
      ...
      .select('id,total_score,recommended_action,scored_at,explanation,products:amm_products!inner(id,title,external_id,current_price,currency,warehouse_country,eu_stock_verified,promotion_link,forecasts:amm_forecasts(horizon_days,forecast_demand,confidence,generated_at))')
